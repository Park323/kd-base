...
def multi_label_accuracy(ans, pred, thresholds:Union[List[float], float]=0.5, method='classwise', average=True)->Union[List[float], float]:
    """
    Refered a paper "A Literature Survey on Algorithms for Multi-label Learning"
    """
    ans = ans.astype(bool)
    ...
    if method=='classwise':
        corrects = (pred == ans).sum(axis=0)
        accs = corrects/len(ans)
        
        TP = ((torch.Tensor(pred)*torch.Tensor(ans)) != 0).sum(axis=0)
        TN = (((torch.logical_not(torch.Tensor(pred).type(torch.bool)))*(torch.logical_not(torch.Tensor(ans).type(torch.bool)))) != 0).sum(axis=0)
        
        FP = ((torch.Tensor(pred)*(torch.logical_not(torch.Tensor(ans).type(torch.bool)))) != 0).sum(axis=0)
        FN = (( (torch.logical_not(torch.Tensor(pred).type(torch.bool)))*torch.Tensor(ans)) != 0).sum(axis=0)

    elif method=='acc':
        corrects = pred & ans
        accs = corrects.sum(axis=0) / masks.sum(axis=0)
